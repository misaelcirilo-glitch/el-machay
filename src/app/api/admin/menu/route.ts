...
export async function GET() {
    ...
    const categories = await db`
        ...
        WHERE is_active = true AND restaurant_id = ${session.restaurantId} ORDER BY sort_order
    `;

    const items = await db`
        SELECT id, category_id, name, description, price, image_url,
               is_spicy, is_vegetarian, is_gluten_free, is_featured, is_available, sort_order
        FROM menu_items WHERE restaurant_id = ${session.restaurantId} ORDER BY sort_order
    `;
